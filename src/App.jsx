...
export default function App() {
  ...
  // --- 1. OTOMATIS LOGIN SEMBUNYI (ANONIM) ---
  useEffect(() => {
  ...
  // --- 2. AMBIL DATA GLOBAL (BISA DIAKSES DARI HP MANA SAJA) ---
  useEffect(() => {
    if (!user) return; // Tunggu sampai login anonim selesai
    
    // Path Global untuk Semua Data Perusahaan
    ...
    const settingsRef = doc(db, 'cv_hamsabaidane_settings', 'pricing');
  ...
  const renderSettings = () => {
    const handleSavePricing = async (e) => {
      e.preventDefault();
      try {
        await setDoc(doc(db, 'cv_hamsabaidane_settings', 'pricing'), { list: pricing });
        alert("Pengaturan harga berhasil disimpan permanen!");
      } catch (error) { 
        console.error(error);
        alert("Gagal menyimpan pengaturan. Pastikan Rules Database Firebase sudah 'allow read, write: if true;'"); 
      }
    };
    ...
    return (
      <div className="max-w-3xl mx-auto animate-in fade-in duration-300">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden p-6">
          <p className="text-slate-500 mb-6">Ubah tarif default di bawah ini. Saat Anda menginput data setoran, harga akan otomatis mengikuti tarif yang Anda simpan di sini.</p>
          <form onSubmit={handleSavePricing} className="space-y-4">
            {pricing.map((item, index) => (
              <div key={index} className="flex items-center justify-between p-4 bg-slate-50 rounded-xl border border-slate-200">
                <span className="font-bold text-slate-700 text-lg flex items-center"><Tag className="w-5 h-5 mr-2 text-blue-500"/> {item.nama}</span>
                <div className="flex items-center">
                  <span className="mr-2 text-slate-500 font-medium">Rp.</span>
                  <input type="number" value={item.harga} onChange={(e) => updatePrice(index, e.target.value)} className="w-32 p-2.5 border border-slate-300 rounded-lg text-right font-bold focus:ring-2 focus:ring-blue-500 outline-none" />
                </div>
              </div>
            ))}
            <div className="pt-6">
              <button type="submit" className="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-3 rounded-xl transition shadow-lg flex justify-center items-center">
